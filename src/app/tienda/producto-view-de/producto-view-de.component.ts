...
@Component({
  selector: 'app-producto-view-de',
  templateUrl: './producto-view-de.component.html',
  styleUrls: ['./producto-view-de.component.scss']
})
export class ProductoViewDeComponent implements OnInit {
  ...
  getProduct(){
    return new Promise( resolve =>{
      this._productServices.get( { where: { id: this.id || 116 } } ).subscribe( res => resolve( res.data[0] ), error => resolve( error ) );
    })
  }
  ...
  suma(){
    ...
    if( this.data.sumAmount >= 6 ) this.data.priceTotal = this.dataPro.pro_vendedorCompra * this.data.sumAmount;
